...
export default function BrowsePage() {
  ...
  const [q, setQ] = useState('');        // simple search filter
  ...
  // Simple case-insensitive filter across visible text fields
  const qx = q.trim().toLowerCase();
  function matchTxt(...vals: (string|undefined|null)[]) {
    if (!qx) return true;
    return vals.some(v => (v||'').toLowerCase().includes(qx));
  }

  return (
    <main style={{ padding: 24, maxWidth: 1100, margin: '0 auto' }}>
      <h1>Browse SSC Framework</h1>
      <p style={{ opacity: 0.8 }}>Read-only explorer of Pillars → Themes → Sub-themes → Standards → Indicators.</p>
      ...
      <div style={{ display:'grid', gap:12 }}>
        {pillars
          .filter(p => matchTxt(p.code, p.name, p.statement, p.description))
          .sort(sortBy('sort_order','code'))
          .map(p => {
            const tForP = (themesByPillar[p.id]||[]).sort(sortBy('sort_order','code'))
              .filter(t => matchTxt(t.code, t.name, t.statement, t.description));
            ...
            return (
              <details key={p.id} open>
                <summary>
                  <b>{p.code}</b> — {p.name}
                  {pDefaults.length>0 && <span style={{ marginLeft:8, fontSize:12, opacity:0.7 }}>· default indicator present</span>}
                </summary>
                  ...
                  <div style={{ display:'grid', gap:8, marginTop:8 }}>
                    {tForP.map(t => {
                      const sForT = (subsByTheme[t.id]||[]).sort(sortBy('sort_order','code'))
                        .filter(st => matchTxt(st.code, st.name, st.statement, st.description));
                            ...
                            <div style={{ display:'grid', gap:6, marginTop:6 }}>
                              {sForT.map(st => {
                                const dForS = (stdsBySub[st.id]||[]).sort(sortBy('sort_order','code'))
                                  .filter(sd => matchTxt(sd.code, sd.statement, sd.notes||''));
